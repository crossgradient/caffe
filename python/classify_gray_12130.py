...
def main(argv):
    ...
    for i in range(0,batches) :
        ...
        inputs = [caffe.io.load_image(im_f,color=False) for im_f in currentBatch.file.values]
        predictions = classifier.predict(inputs, False, False)
        w.append(np.argmax(predictions,axis=1))
        f.append(currentBatch.file.values)
        print "Done in %.2f s." % (time.time() - start)

    preds_df = pd.DataFrame(np.transpose([f,w]),columns=['file','label']) 
    preds_df['fullfile'] = preds_df['file'].apply(lambda(x): '../../data/12130/test/' + x)
    preds_df.to_csv('preds.txt',index=False,header=False, sep=' ',columns=['fullfile','label'])
